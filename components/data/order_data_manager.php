...
// *******************************************************************************************************************
  ...
  // *******************************************************************************************************************
  // Create a payment request specifically for updating a payment card associated with an existing subscription.
  // The $nets_subscription_id is the Nets subscription ID (not the internal subscription ID).
  // Returns the payment data from Nets, or null if the payment creation failed.
  //
  // See the Nets documentation at:
  // https://developer.nexigroup.com/nexi-checkout/en-EU/api/payment-v1/#v1-payments-post
  public function create_payment_for_card_update($nets_subscription_id)
  {
    if (empty($nets_subscription_id)) {
      error_log('Failed to create payment for card update: nets_subscription_id not provided');
      return null;
    }

    // Read user data
    $user_id = get_current_user_id();
    $buyer = User_Data_Manager::get_user_data($user_id);
    if ($buyer === null) {
      error_log('Failed to create payment for card update: User data not found for user ID ' . $user_id);
      return null;
    }

    // Create payload for Nets API with zero amount order and subscription ID
    $payload = array(
      'checkout' => array(
        'integrationType' => 'HostedPaymentPage',
        'returnUrl' => Utility::get_domain() . '/subscription/html/card_update_complete.php',
        'cancelUrl' => Utility::get_domain() . '/subscription/html/user_dashboard.php',
        'termsUrl' => Utility::get_gibbs_terms_url(),
        'merchantHandlesConsumerData' => true,
        'consumer' => array(
          'reference' => $buyer['user_id'],
      ...
      'order' => array(
        ...
        'amount' => 0,
        'currency' => 'NOK',
        'reference' => 'card-update-' . time()
      ),
      'subscription' => array(
        'id' => $nets_subscription_id,
        'endDate' => null
      )
    );
    ...
    // Create and submit a request to Nets
    $settings = Settings_Manager::read_settings($this->access_token);
    $request = curl_init($settings->get_nets_payment_url());
    curl_setopt_array($request, array(
      CURLOPT_CUSTOMREQUEST => 'POST',
    ...
    if (($http_code === false) || !is_numeric($http_code)) {
      error_log('Failed to create payment for card update: curl_getinfo failed to return HTTP response code.');
      return null;
    }
    
    if ($response === false) {
      error_log('Failed to create payment for card update: cURL error: ' . $error);
      return null;
    }
    ...
    switch ($http_code) {
      case 201:
        ...
        return $response;
      case 400:
        // HTTP 400 Bad request: The request was not valid. Log errors.
        error_log('Failed to create payment for card update: Nets returned HTTP 400 Bad request. Errors: ' . print_r($response, true));
        return null;
      case 500:
        // HTTP 500 Internal server error
        error_log('Failed to create payment for card update: Nets returned HTTP 500 Internal server error. Errors: ' .
          print_r($response, true));
        return null;
      default:
        // Other HTTP error
        error_log('Failed to create payment for card update: Nets returned HTTP ' . $http_code);
        return null;
    }
  }
