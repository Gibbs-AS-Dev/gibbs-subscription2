	...
	protected function process_redirect_handler( $order_id ) {
		...
		if(isset($response['paymentId'])){
			...
			$order->save();
				...
				WC()->session->set( 'dibs_payment_id' ,$response['paymentId'] );
				...
				update_post_meta( $order_id, 'return_url', WC()->session->get( 'dibs_return_url' ) );
	
				return array(
					'result'   => 'success',
					'redirect' => get_option('siteurl')."/gibbspay.php?order_id=".base64_encode($order_id),
				);
