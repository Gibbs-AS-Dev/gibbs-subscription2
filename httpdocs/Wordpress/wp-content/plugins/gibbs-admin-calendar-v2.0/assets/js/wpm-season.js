      ...
      //  console.log(event)
        ...
        if(event.org_data && event.org_data.name && event.org_data.name != ""){
            left_info_data.append('<p class="tooltip-custom listing_div">'+
                    '<b>Annonse:</b> <span>'+event.org_data.name+'</span>'+
                '</p>')
        }
 ...
 */
    ...
    if(!Array.isArray(calendarShowAdminIcons)){
        calendarShowAdminIcons = ["date","time","listing","repeated","linked","not_repeated","not_linked","comment","notes","custom_field"];
    }
  ...
  //  newResources = section_resources;
    ...
    window.calendar = calendar = $('#scheduler').mobiscroll().eventcalendar({
        ...
        renderHeader: function () {
            ...
            renderHeader += '<div class="cal_typee">'+
                '<select class="k-dropdown change_season_view">' +
                        '<option value="forespurte" '+(season_view == 'forespurte'?'selected':'')+'>Forespurte bookinger</option>' +
                        '<option value="algoritme" '+(season_view == 'algoritme'?'selected':'')+'>Algoritme forslag</option>' +
                        '<option value="manuelle" '+(season_view == 'manuelle'?'selected':'')+'>Manuelle endringer</option>' +
                '</select>'+
            '</div>';
            var options_season = "";
            if(Array.isArray(seasons_data)){
                seasons_data.forEach(function (seasons_item) {
                    if(selected_season == seasons_item.id){
                        var selected_s = "selected";
                    }else{
                        var selected_s = "";
                    }
                    options_season += '<option value="'+seasons_item.id+'" '+selected_s+'>'+seasons_item.name+'</option>';
                });
            }

            
            renderHeader += '<div class="cal_typee">'+
                        '<select class="k-dropdown season_change">' +
                                options_season +
                        '</select>'+
                    '</div>';
            if(season_view == "forespurte"){
                   renderHeader += '<div class="cal_typee"><button class="k-button header_btns move_raw_booking">Hopp over algoritme</button></div>';
            }
            if(season_view == "algoritme"){
                renderHeader += '<div class="cal_typee"><button class="k-button header_btns algorithm_popup_btn">Gjør fordeling <i class="fa-solid fa-wand-magic-sparkles"></i></button></div>';

                renderHeader += '<div class="cal_typee"><button class="k-button header_btns move_approve_booking">Overfør fordeling</button></div>';
            }
            if(season_view == "manuelle"){    

                renderHeader += '<div class="cal_typee">'+
                                    '<select class="k-dropdown export_select header_btns">' +
                                            '<option value="">'+translations["select_export_option"]+'</option>' +
                                            '<option value="export">'+translations["export_in_database"]+'</option>' +
                                            '<option value="export_csv">'+translations["export_in_csv"]+'</option>' +
                                    '</select>'+
                                '</div>';
            ...
            renderHeader += '<input type="text" placeholder="Søk..." id="scheduler-search-input" style="width: 20%" />';
            ...
            // View select
            renderHeader += '<div class="fc-button-group cal_view_select_outer""><select id="cal_view_select" class="cal_view_select">';
            if(type_of_form != "1"){
                renderHeader +=  '<option value="timeline_day" class="fc-timelineDay-button fc-button fc-state-default" ' + (calendar_view_val === 'timeline_day' ? 'selected' : '') + '>Tidslinje dag</option>';
            }    
            renderHeader += '<option value="timeline_week" class="fc-timelineWeek-button fc-button fc-state-default" ' + (calendar_view_val === 'timeline_week' ? 'selected' : '') + '>Tidslinje uke</option>';
            if(type_of_form != "1"){
                renderHeader += '<option value="timeline_month"class="fc-timelineMonth-button fc-button fc-state-default" ' + (calendar_view_val === 'timeline_month' ? 'selected' : '') + '>Tidslinje måned</option>';
              /*   renderHeader += '<option value="timeline_year" class="fc-timelineYear-button fc-button fc-state-default" ' + (calendar_view_val === 'timeline_year' ? 'selected' : '') + '>Tidslinje år</option>'; */
            
                renderHeader += '<option value="schedule_day" class="fc-scheduleDay-button fc-button fc-state-default" ' + (calendar_view_val === 'schedule_day' ? 'selected' : '') + '>Dag</option>';
            }
            renderHeader += '<option value="schedule_week" class="fc-scheduleWeek-button fc-button fc-state-default" ' + (calendar_view_val === 'schedule_week' ? 'selected' : '') + '>Uke</option>';
            if(type_of_form != "1"){
                renderHeader += '<option value="schedule_month"class="fc-scheduleMonth-button fc-button fc-state-default" ' + (calendar_view_val === 'schedule_month' ? 'selected' : '') + '>Måned</option>';
                renderHeader += '<option value="schedule_year" class="fc-scheduleYear-button fc-button fc-state-default" ' + (calendar_view_val === 'schedule_year' ? 'selected' : '') + '>År</option>';
                if(type_of_form != "2"){
                    renderHeader += '<option value="agenda" class="fc-agenda-button fc-button fc-state-default" ' + (calendar_view_val === 'agenda' ? 'selected' : '') + '>Agenda</option>';
                }
            }    
            renderHeader += '</select></div></div>';
            renderHeader += '</div>';
    ...
    jQuery(document).on('change',".export_select",function(){
        ...
        if(this.value != ""){
            ...
            if(export_type == "export"){
                ...
                setTimeout(function(){
                    jQuery("body").find(".mbsc-alert-message").prepend("<div class='input-emaill'><input type='checkbox' class='input-email-checkbox' name='input-email-checkbox'> <span for='input-email-checkbox'>Send Email?</span></div>")
                },100)

                jQuery(document).on("change",".input-email-checkbox",function(){
