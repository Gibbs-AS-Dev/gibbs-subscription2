...
class Listeo_Core_Messages {
	...
	*/
	public function get_conversations( $user_id, $limit = '', $offset = '')  {
        ...
        $user_id_cr = get_current_user_id(); 
        ...
        SELECT * FROM `" . $wpdb->prefix . "listeo_core_conversations` 
        WHERE  user_1 = '$user_id' OR user_2 = '$user_id'
        OR  user_1 = '$user_id_cr' OR user_2 = '$user_id_cr'
        ORDER BY last_update DESC $limit $offset
        ");
	...
	public function get_conversations_referral( $referral, $limit = '', $offset = '')  {
...
//     }
    ...
    public function get_conversation_ajax() {
		...
		if($group_admin != ""){
            ...
            foreach ($conversation as $key => $message) {
             ...
             ?>
                <div class="message-bubble <?php if(get_current_user_id()  == $message->sender_id ) echo esc_attr('me'); ?>">
    ...
    public  function get_single_conversation( $user_id, $conversation_id)  {
        ...
        $result_conv = $wpdb->get_var( "
            SELECT GROUP_CONCAT(c2.id) 
            FROM `" . $wpdb->prefix . "listeo_core_conversations` c1
            JOIN `" . $wpdb->prefix . "listeo_core_conversations` c2 
                ON c1.referral = c2.referral
            WHERE c1.id = '$conversation_id'
        ");

        if(!empty($result_conv)){
            
            $result  = $wpdb -> get_results( "
            SELECT * FROM `" . $wpdb->prefix . "listeo_core_messages` 
            WHERE  conversation_id in ($result_conv)
	...
	*/
    ...
    public function conversation_exists($recipient,$referral){
        ...
        if($referral != ""){
            ...
            $conversations = $this->get_conversations_referral($referral);
        ...
        foreach ($conversations as $key => $conv) {
            if($user_id == (string)$conv->user_1 && $recipient == (string)$conv->user_2 && $referral == $conv->referral){
                ...
                return $conv->id;
            }elseif ($group_admin == (string)$conv->user_1 && $recipient == (string)$conv->user_2 && $referral == $conv->referral) {
                return $conv->id;
            }elseif ($group_admin == $conv->user_2 && $recipient == $conv->user_1  && $referral == $conv->referral) {
                return $conv->id;
            }
        }
