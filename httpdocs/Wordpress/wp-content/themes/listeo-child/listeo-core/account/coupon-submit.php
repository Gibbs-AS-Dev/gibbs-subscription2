	...
	<form action="<?php echo get_permalink($coupons_page); ?>" method="post" id="submit-listing-form" class="listing-manager-form" enctype="multipart/form-data">
		...
		<div class="add-listing-section row">
			<div class="add-listing-headline">
				<h3>  <?php esc_html_e('General Coupon Settings','listeo_core'); ?></h3>
			</div>
			<div class="col-md-12">
		...
		<!-- 	<div class="col-md-12">
			...
			<div id="general_coupon_data" class="panel woocommerce_options_panel" style="display: block;">
				...
				<div class="col-md-3">
					<p class="form-field start_date_field ">
						<label for="date_start"><?php esc_html_e('Coupon start date','listeo_core'); ?></label>
						<?php 
							if ( isset( $data->coupon_edit ) && $data->coupon_edit ) { 
								$date_start = get_post_meta( $coupon_id, "date_start", true ); 
							
							} ?>
						<input type="text" class="input-date" style="" name="date_start" autocomplete="off" id="date_start" value="<?php if( isset( $data->coupon_edit ) && $date_start ) echo date('Y-m-d', strtotime($date_start));  ?>" placeholder="YYYY-MM-DD" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])"> 
					</p>			
				</div>
		...
		<div class="add-listing-section row">
			<div class="add-listing-headline">
				<h3>  <?php esc_html_e('Begrensninger','listeo_core'); ?></h3>
			</div>
		<div id="usage_restriction_coupon_data" class="panel woocommerce_options_panel" style="display: block;">
				...
				<p class="form-field">
					...
					?>
					<label><?php esc_html_e('For products:','listeo_core'); ?><i class="tip" data-tip-content="<?php esc_html_e('La det stå tomt her, om kupongen skal være til alle dine annonser','listeo_core'); ?>"></i></label>
					<select class="select2-single" multiple="" style="width: 50%;" name="listing_ids[]" 
					data-placeholder="<?php esc_html_e('Søk..	','listeo_core'); ?>" data-action="" tabindex="-1" aria-hidden="true">
				...
				<!-- <div class="col-md-12">
					<p class="form-field customer_email_field ">
						<label for="customer_email"><?php esc_html_e('Velg eposter som kan bruke denne kupongen','listeo_core'); ?></label>
				...
				<div id="usage_limit_coupon_data" class="panel woocommerce_options_panel" style="display: block;">
					<div class="col-md-12">
						<p class="form-field usage_limit_field ">
							<label for="usage_limit"><?php esc_html_e('Antall ganger en kupong kan brukes','listeo_core'); ?></label>
							<input type="number" class="short" style="" name="usage_limit" id="usage_limit" value="<?php if(isset($coupon_id)) echo esc_html(get_post_meta($coupon_id,'usage_limit',true)); ?>" placeholder="<?php esc_html_e('Ubegrenset','listeo_core'); ?>" step="1" min="0"> 
						</p>
					</div>
...
<!-- 		<div class="add-listing-section row">
			<div class="add-listing-headline">
				<h3>  <?php esc_html_e('Usage limits','listeo_core'); ?></h3>
			</div>
			
				
					<div class="col-md-6">
						<p class="form-field usage_limit_per_user_field ">
							<label for="usage_limit_per_user"><?php esc_html_e('Antall ganger samme bruker kan bruke kupongen','listeo_core'); ?></label>
	...
	<p>
		...
		<button type="submit" value="Submit Coupon" name="submit_coupon"  class="button margin-top-20"><i class="fa fa-circle-arrow-right"></i> 
