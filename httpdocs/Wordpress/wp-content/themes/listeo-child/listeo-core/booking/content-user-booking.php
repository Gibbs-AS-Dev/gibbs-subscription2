...
switch ($data->status) {
	...
	case 'paid' :
		...
		$tag[] = '<span class="booking-status">'.esc_html__('Approved', 'listeo_core').'</span>';
		if($data->price>0){
			$ifInvoice =  get_post_meta($data->ID,'invoice_payment',true);
			if($ifInvoice == 'true'){
				$tag[] = '<span class="booking-status paid">'.esc_html__('Payment by invoice', 'listeo_core').'</span>';
			}else{
		...
		<div class="list-box-listing-content">
			<div class="inner">
				...
				<div class="inner-booking-list">
					...
					<ul class="booking-list" id="location">
						<?php echo $address; ?>
				...
				<div class="inner-booking-list" style="display:none;">
...
<!--							--><?php //echo date_i18n(get_option( 'time_format' ), strtotime($date_created[1])); } ?>
	...
	<div class="buttons-to-right">
		...
		<?php endif; ?>
		<?php if($show_reject) : ?>
		<a class="button gray approvebtn" style="display: none;"><i class="sl sl-icon-check"></i> Godkjenn</a>
...
<script>
	jQuery(document).ready(function(){
		jQuery('.buttons-to-right p a').attr('class','button green pay-invoice');
	});
    jQuery('.buttons-to-right').each(function (){
        var id = jQuery(this).parent().attr('id');
        var text1 = jQuery(`#${id} #date`).text();
        var text1Color = jQuery(`#${id} #date`).css('color');
        var adres = jQuery(`#${id} #client #billing_address_1`).text();
		text1 = text1.trim();
		jQuery(`#${id} #location`).parent().hide();
		jQuery(`#${id} #client #name`).parent().parent().hide();
		if( text1 == '1, January, 1970 00:00 - 1, January, 1970 24:00' || text1 == '1, January, 1970 12:00 - 1, January, 1970 24:00' || text1 == 'January 1, 1970 at 00:00 - 00:00' || text1 == '01/01/1970 at 12:00 am - 12:00 am' || text1 == '30/11/-0001 at 12:00 am - 12:00 am' || text1 =='01/01/1970 00:00 - 01/01/1970 24:00' || text1 =='30/11/-0001 00:00 - 30/11/-0001 24:00'){
            jQuery(`#${id} #date`).parent().parent().css('display', 'none');
			jQuery(`#${id} #client #billing_address_1`).parent().parent().css('display', 'none');            jQuery(`#${id} #price`).parent().parent().css('display', 'none');
            if(adres == 'true ' && text1Color != 'rgb(95, 144, 37)'){
                jQuery(`#${id} .booking-status.pending`).text('Trenger din oppmerksomhet')
                jQuery(`#${id} #price`).parent().parent().css('display', 'block');
                jQuery(this).find('.approvebtn').css('display','');
            }
            if(adres == 'true ' && text1Color == 'rgb(95, 144, 37)'){
                jQuery(`#${id} #price`).parent().parent().css('display', 'block');
            }
