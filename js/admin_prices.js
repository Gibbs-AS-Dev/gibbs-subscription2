...
var nameEdit, startDateEdit, editStartDateButton, closeStartDateButton, startDateCalendarBox,
  endDateEdit, editEndDateButton, closeEndDateButton, endDateCalendarBox, productTypesBox,
  locationsBox, priceModEditorBox, errorMessageBox, submitButton;
...
function initialise()
{
  ...
  if (resultCode >= 0)
    alert(getText(20, 'Det oppstod en feil. Vennligst kontakt kundeservice og oppgi feilkode $1.',
      [String(resultCode)]));
}
...
function displayCapacityPriceRules()
{
  ...
  o = new Array((capacityPriceRules.length * 15) + 16); // *** //
  p = 0;
  ...
  o[p++] = getText(3, 'Navn');
  o[p++] = '</th><th>';
  o[p++] = getText(4, 'Fra dato');
  o[p++] = '</th><th>';
  o[p++] = getText(5, 'Til dato');
  o[p++] = '</th><th>';
  o[p++] = getText(6, 'Lager');
  o[p++] = '</th><th>';
  o[p++] = getText(7, 'Bodtyper');
  o[p++] = '</th><th>';
  o[p++] = getText(8, 'Rediger');
  o[p++] = '</th><th>';
  o[p++] = getText(9, 'Slett');
  ...
  for (i = 0; i < capacityPriceRules.length; i++)
  {
    ...
    o[p++] = '</td><td';
    o[p++] = style;
    o[p++] = '><button type="button" class="icon-button" onclick="editPriceRule(RULE_TYPE_CAPACITY, ';
    o[p++] = String(i);
    o[p++] = ');"><i class="fa-solid fa-pen-to-square"></i></button></td><td';
    o[p++] = style;
    o[p++] = '><button type="button" class="icon-button" onclick="deleteCapacityPriceRule(';
    o[p++] = String(i);
...
function getCapacityPriceMods(index)
{
  ...
  // Headline.
  o[p++] = '<tr class="price-mods"><td colspan="8" class="price-mods"><div class="price-mods"><table cellspacing="0" cellpadding="0"><thead><tr><th>';
...
function displaySpecialOfferPriceRules()
{
  ...
  o = new Array((specialOfferPriceRules.length * 15) + 16); // *** //
  p = 0;
  ...
  for (i = 0; i < specialOfferPriceRules.length; i++)
  {
    ...
    o[p++] = '</td><td';
    o[p++] = style;
    o[p++] = '><button type="button" class="icon-button" onclick="editPriceRule(RULE_TYPE_SPECIAL_OFFER, ';
    o[p++] = String(i);
    o[p++] = ');"><i class="fa-solid fa-pen-to-square"></i></button></td><td';
    o[p++] = style;
    o[p++] = '><button type="button" class="icon-button" onclick="deleteSpecialOfferPriceRule(';
    o[p++] = String(i);
    o[p++] = ');"><i class="fa-solid fa-trash"></i></button></td></tr>';
    // Write table of price lines, if the user has opened the box.
    if (specialOfferPriceRules[i][c.pru.OPEN])
...
function editPriceRule(ruleType, index)
{
  ...
  // Obtain pointers to user interface elements.
  Utility.readPointers(['nameEdit', 'startDateEdit', 'editStartDateButton', 'closeStartDateButton',
    'startDateCalendarBox', 'endDateEdit', 'editEndDateButton', 'closeEndDateButton',
    'endDateCalendarBox', 'productTypesBox', 'locationsBox', 'priceModEditorBox', 'errorMessageBox',
    'submitButton']);
...
function getEditPriceRuleContent()
{
  ...
  // Content.
  o[p++] = '<div class="dialogue-content"><form action="">';
  ...
  if (editedRuleType === RULE_TYPE_SPECIAL_OFFER)
  {
    ...
    o[p++] = getText(42, 'Sett varigheten til 0 for &aring; gi permanent rabatt.');
    o[p++] = '</div>';
  }
...
function getEditPriceRuleName()
{
  ...
  o[p++] = getText(32, 'Navn:');
  o[p++] = '</label><input type="text" id="nameEdit" class="long-text" onkeyup="enableSubmitButton();" onchange="enableSubmitButton();"';
  if (!isNew)
  {
    o[p++] = ' value="';
...
function getEditPriceRuleStartDate()
{
  ...
  o[p++] = '</label><input type="text" id="startDateEdit" readonly="readonly" class="long-text" ';
  if (!isNew)
  {
    o[p++] = 'value="';
...
function getEditPriceRuleEndDate()
{
  ...
  o[p++] = '</label><input type="text" id="endDateEdit" readonly="readonly" class="long-text" ';
  if (!isNew)
  {
    o[p++] = 'value="';
...
function getEditPriceRuleProductTypeColumn()
{
  ...
  o = new Array((productTypes.length * 13) + 12);
  p = 0;

  // All product types radio button.
  o[p++] = '<div class="column for-product-types-column"><div class="form-element"><input type="radio" id="allProductTypesRadio" name="for_all_product_types" value="1" onchange="toggleProductTypesBox(this);"';
  if (isNew)
  ...
  o[p++] = '</ul></div></div></div>'; // End of: indented-box, form-element, column
...
function getEditPriceRuleLocationColumn()
{
  ...
  o = new Array((locations.length * 13) + 12);
  p = 0;

  // All locations radio button.
  o[p++] = '<div class="column for-locations-column"><div class="form-element"><input type="radio" id="allLocationsRadio" name="for_all_locations" value="1" onchange="toggleLocationsBox(this);"';
  if (isNew)
    forLocations = null;
  else
...
function getCapacityPriceModEditBoxes(index)
{
  ...
  o[p++] = String(index);
  o[p++] = '_minCapacity" class="numeric spaced" value="';
  ...
  o[p++] = '<input type="number" id="priceRule';
  o[p++] = String(index);
  o[p++] = '_maxCapacity" class="numeric spaced" value="';
  ...
  o[p++] = '<input type="number" id="priceRule';
  o[p++] = String(index);
  o[p++] = '_modifier" class="numeric spaced" value="';
...
function getSpecialOfferPriceModEditBoxes(index)
{
  ...
  o[p++] = ' <input type="number" id="priceRule';
  o[p++] = String(index);
  o[p++] = '_duration" class="numeric spaced" value="';
...
function updatePriceModTable()
{
  ...
  o = new Array(editedPriceMods.length + 1);
  p = 0;
