...
function cancelSubscription(id)
{
  ...
  if (index >= 0)
  {
    ...
    if (approved)
    {
      ...
      o[p++] = '<form id="cancelSubscriptionForm" action="/subscription/html/user_dashboard_old.php" method="post"><input type="hidden" name="action" value="cancel_subscription" />';
      o[p++] = Utility.getHidden('id', subscriptions[index][c.sub.ID]);
      o[p++] = '</form>';
...
function displaySubscriptions()
{
  ...
  if (subscriptions.length <= 0)
  {
    subscriptionsBox.innerHTML = '<div class="form-element"><p>' +
      getText(9, 'Velkommen som bruker av Gibbs minilager! Du har ingen lagerboder i &oslash;yeblikket. Hvis du allerede har sendt foresp&oslash;rsel, er det bare &aring; vente til vi kontakter deg. Hvis ikke, klikk Bestill lagerbod for &aring; komme i gang.') + '</p></div>'
...
function displaySubscriptionSet(indexes)
{
  ...
  o = new Array((indexes.length * 21) + 18);
  p = 0;
  
  o[p++] = '<table cellspacing="0" cellpadding="0"><thead><tr><th>';
  o[p++] = getText(3, 'Lagerbod (klikk for pris)');
  o[p++] = '</th><th>';
  ...
  o[p++] = '</th><th>';
  o[p++] = getText(24, 'Forsikring (klikk for pris)');
  o[p++] = '</th><th>';
  ...
  for (i = 0; i < indexes.length; i++)
  {
    ...
    o[p++] = '</td><td>';
    o[p++] = Utility.getProductTypeName(subscriptions[index][c.sub.PRODUCT_TYPE_ID]);
    o[p++] = '</td><td>';
    o[p++] = Utility.getStatusLabel(st.sub.TEXTS, st.sub.COLOURS, subscriptions[index][c.sub.STATUS]);
    o[p++] = '</td><td>';
